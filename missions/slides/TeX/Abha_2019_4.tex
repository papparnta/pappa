\documentclass{beamer}%,[handout] %,hyperref={pdfpagelabels=false},draft,handout,handout
...
$\!\!\!m(x,y)=\left(x - \frac {\psi_{m-1} \psi_{m+1}}{\psi_{m}^{2}(x)}, \frac{\psi_{2 m}(x,y)}{2\psi_{m}^{4}(x)} \right)$\end{beamercolorbox}}
\end{theorem}
\end{frame}
...
\begin{beamercolorbox}[rounded=true,shadow=true,wd=12cm,center]{postit}
 \small{$E:y^2+xy=x^3+x^2+1\ \Rightarrow\quad
E(\F_{2^{100}})=2^{100}+1-\left(\frac{1+\sqrt{-7}}2\right)^{100}-
\left(\frac{1-\sqrt{-7}}2\right)^{100}$}\pause

\hspace*{6cm}\ =\small{1267650600228229382588845215376}
\end{beamercolorbox}
\end{frame}
...
\hfill where $P_E(T)=T^2-aT+q=(T-\alpha)(T-\beta)\in\Z[T]$\pause
\begin{block}{Curves $/\F_3$}\vspace*{-9.9mm}
\begin{center}
...
$s_0=2,s_1=a$ and $s_{n+1}=as_n-qs_{n-1}$
\end{lemma}
\end{frame}
...
\begin{theorem} Let $E:y^2=x^3+Ax+B$ over $\F_q$. Then\qquad
...
\alert{\centerline{$\vspace*{-8mm}1+\left(\frac{x_0^3+Ax_0+B}{\F_q}\right)=\begin{cases}
...
Hence\qquad
{{$\#E(\F_q)=1+\sum_{x\in\F_q}\left(1+\left(\frac{x^3+Ax+B}{\F_q}\right)\right)$}}\vspace*{-2.7pt}
\end{proof}
\end{frame}
...
\vspace*{-5mm}
\begin{proof}
\alert{\vspace*{-0.8cm}\begin{align*}
