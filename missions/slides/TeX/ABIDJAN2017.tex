\documentclass[12pt,handout]{beamer} %,hyperref={pdfpagelabels=false},draft,handout,handout
\usepackage[orientation=landscape,size=custom,width=16,height=9,scale=0.50,debug]{beamerposter} 
\usepackage[english]{babel}
\usepackage{lmodern}% http://ctan.org/pkg/lm
\usepackage[latin1]{inputenc}
\usepackage{times,hyperref,tikz,colortbl,yfonts,translator} %yfonts,
...
\begin{frame}
\includegraphics[width=1.6cm]{images/roma3.pdf}\hfill\includegraphics[width=1.9cm]{images/UFHB.jpg}
\vfill

\begin{center}\begin{sc}
\begin{Large}

\textcolor{red}{Elliptic curves an elementary approach}
\end{Large}\bigskip

\ {Francesco Pappalardi}\bigskip\bigskip


April $13^{\text{th}}$ 2017\medskip
\vfill
\end{sc}\end{center}

%\includegraphics[width=1.6cm]{images/cimpalogo.pdf}\hfill
\begin{minipage}[b]{9.3cm}
\textbf{Universit\'e F\'elix Houphou\"et Boigny}\\  %Монгол Улсын Их Сургууль
Th\'eorie Alg\'ebrique des nombres et applications notamment \`a la cryptographie\\
...
The \emph{discriminant} of a Weierstra\ss\ equation over any field $K$ is
\centerline{\begin{beamercolorbox}[shadow=true,center,rounded=true,wd=14cm]{formul}
\begin{align*}
D_E&:=-\left(-a_1^5 a_3 a_4 - 8 a_1^3 a_2 a_3 a_4 - 16 a_1 a_2^2 a_3 a_4 + 36 a_1^2 a_3^2 a_4\right.  \\
...
\begin{Note}
 $E$ is \emph{non singular} if and only if $D_E\neq0$
\end{Note}
\end{frame}
...
After a suitable affine transformation we can assume that $E/K$
has a \emph{Special Weierstra\ss}\ equation:\pause
\begin{scriptsize}
 \begin{example}[Classification]
\centerline{\begin{tabular}{|l|c|l|}
\hline
 $E$ & $p=\operatorname{char}K$ & $D_E$\\
\hline
 $y^2=x^3+Ax+B$ & $\ge5$ & $-16(4A^3+27B^2)$\\
& or $=0$&\\
...
$E: y^2+a_1xy+a_3y=x^3+a_2x^2+a_4x+a_6$\end{beamercolorbox}\quad $P_1 = (x_1, y_1), P_2 = (x_2, y_2)\in E(K)\setminus\{\infty\}$,}
...
\begin{beamercolorbox}[shadow=true,center,rounded=true,wd=5cm]{formul}$P_1 +_E P_2 = \infty$
\end{beamercolorbox}
 \item $x_1 \neq x_2$\\
...
\item If $P_1 = P_2$
\begin{itemize}
 \item $2y_1+a_1x_1+a_3 = 0\ \hfill\Rightarrow\hfil$\ \ \
\begin{beamercolorbox}[shadow=true,center,rounded=true,wd=5cm]{formul}$P_1 +_E P_2 = 2P_1 = \infty$\end{beamercolorbox}
\item $2y_1+a_1x_1+a_3\neq 0$\\
\centerline{\begin{beamercolorbox}[shadow=true,center,wd=12cm]{postit}
...
Then\\
\centerline{\begin{beamercolorbox}[shadow=true,center,rounded=true,wd=13cm]{formul}
...
\scriptsize{
\centerline{\begin{beamercolorbox}[shadow=true,center,rounded=true,wd=10cm]{formul}
$E: y^2=x^3+Ax+B$\end{beamercolorbox}\quad $P_1 = (x_1, y_1), P_2 = (x_2, y_2)\in E(K)\setminus\{\infty\}$,}
\begin{beamerboxesrounded}[upper=block title example,lower=block body alerted,shadow=true]{Addition Laws for  the sum of affine points}
\begin{itemize}
 \item If $P_1\neq P_2$
\begin{itemize}
...
\hline
\end{tabular}}
\ \hfill \begin{beamercolorbox}[center,wd=11cm]{postit}
...
\small{\begin{block}{Groups of points of curves over $\F_3$}\centerline{
...
\begin{small}
(\alert{12 $E/\F_5$})\hfil $(2\le\#E(\F_5)\le10$, \alert{$8$ values})\hfil $\forall n\in\{2,3,5,7,10\}\exists! E/\F_5: \alert{\#E(\F_5)\cong C_n}$\pause

\begin{example}[Curves with $\#E(\F_5)\in\{4,6,8,9\}$]
\begin{itemize}[<+-| alert@+>]
 \item \alert{$E_1: y^2=x^3+1$} and \alert{$E_2: y^2=x^3+2$}\hfill order $6$\\
 \begin{columns}
\begin{column}{2cm}
\tiny{\begin{beamercolorbox}[shadow=true,center,rounded=true,wd=2.5cm]{postit}
        $\begin{cases}
x\longleftarrow 2x\\
y\longleftarrow \sqrt{3}y
 ...
 \end{column}
 \begin{column}{8cm}
$E_1\cong_{\F_{25}}E_2$\quad  \alert{($j(E_1)=j(E_2)=0$)}
 \end{column}
 \end{columns}
\item \alert{$E_3: y^2=x^3+x$} and \alert{$E_4: y^2=x^3+x+2$}
\hfill order $4$
$$E_3(\F_5)\cong C_2\oplus C_2\ \alert{(j(E_3)=1728=3)}\qquad E_4(\F_5)\cong C_4\ \alert{(j(E_4)=1)}$$
\item \alert{$E_5: y^2=x^3+4x$} and \alert{$E_6: y^2=x^3+4x+1$}
\hfill order $8$
$$E_5(\F_5)\cong C_2\oplus C_4\ \alert{(j(E_5)=3)}\qquad E_6(\F_5)\cong C_8\ \alert{(j(E_6)=1)}$$
\item \alert{$E_7: y^2=x^3+x+1$}\qquad  order $9$ and $E_7(\F_5)\cong C_9$ \alert{($j(E_7)=2$)}
\end{itemize}
\end{example}
\end{small}
\end{frame}
...
\centerline{
 \begin{beamercolorbox}[rounded=true,shadow=true,wd=10cm,center]{postit}
...
\begin{itemize}[<+-| alert@+>]
 ...
 \item roots are distinct since discriminant $D_E\neq0$
...
So, if $p>3$ and $E: y^2=x^2+Ax+B$
 ...
 \begin{beamercolorbox}[rounded=true,shadow=true,wd=14cm,center]{formul}
{\small $2P=(x_{2P},y_{2P})=2(x_1,y_1)=({\color[cmyk]{0,1,1,0.5}\lambda^2-2x_1},
...
\begin{beamercolorbox}[rounded=true,shadow=true,wd=12cm,center]{postit}
...
\centerline{\begin{beamercolorbox}[rounded=true,shadow=true,wd=6.5cm,center]{formul}
$E[m]\cong C_m\oplus C_m$\end{beamercolorbox}}

If $m=p^rm', p\nmid m'$,

\centerline{\begin{beamercolorbox}[rounded=true,shadow=true,wd=9cm,center]{formul}
...
\centerline{\begin{beamercolorbox}[rounded=true,shadow=true,wd=13.2cm,center]{formul}
