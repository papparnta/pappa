...
\centerline{\begin{beamercolorbox}[shadow=true,center,rounded=true,wd=11cm]{postit}
...
\ \hfill\scriptsize{$3P=(34,25), 4P=(8,6), 5P=(16,19),\ldots 3P+4Q=(31,28),\ldots$}
 ...
 %$\bullet$ Show that if $E_1/\F_p$ is equivalent to $E_2/\F_p$, then $E_1(\F_{q^n})\cong E_2(\F_{q^n})\forall n\in\N$.
...
\begin{itemize}[<+-| alert@+>]
 \item \alert{$E_1: y^2=x^3+1$} and \alert{$E_2: y^2=x^3+2$}\hfil both order $6$ and $E_1(\F_5)\cong E_2(\F_5)\cong C_6$
%  \begin{columns}
% \begin{column}{4cm}
...
% Let $P=(x_1,y_1)\in E(\F_p)\setminus\{\infty\},$\\ \pause
% \centerline{
...
% \begin{Note}
% \begin{itemize}[<+-| alert@+>]
%  \item the number of points of order $2$ in $E(\F_p)$ equals the number of roots of $X^3+Ax^2+Bx+C$ in $\F_p$
%  \item roots are distinct since discriminant ${D}_E\neq0$
% % \item $E(\F_{q^6})$ has always $3$ points of order $2$ if $E/\F_p$
...
$E[2]=\{P\in E(\overline{\F_p}): 2P=\infty\}.$
...
\begin{corollary} Let $E/\F_p$. $\exists n,k\in\mathbb N$ are such that
\centerline{\begin{beamercolorbox}[rounded=true,shadow=true,wd=6cm,center]{formul}
$$E(\F_p)\cong C_n\oplus C_{nk}$$\end{beamercolorbox}}
\end{corollary}\pause
...
%
%
% The $\ell$-torsion group of $E/\overline{\F_p}$ is isomorphic to
...
\vspace*{-6mm}So \alert{$\#E(\F_q)\in [(\sqrt q -1)^2, (\sqrt q+1)^2]$} the \emph{Hasse interval} ${\mathcal I}_q$\pause

\vspace*{-1mm} \begin{example}[Hasse Intervals]
\begin{scriptsize}
...
% \begin{frame}
% \begin{theorem}[R\"uck]
% Suppose $N$ is a possible order of an elliptic curve $/\F_p$,  $q=p^n$.  Write
% 
% \centerline{
% $N = p^e n_1 n_2,\quad p\nmid n_1 n_2\quad\text{and}\quad n_1\mid n_2\ (\text{possibly }n_1 = 1).$}
% 
% There exists $E/\F_p$ s.t.
% $$E(\F_p)\cong C_{n_1}\oplus C_{n_2p^e}$$
% if and only if
% \begin{enumerate}[<+-| alert@+>]
...
% \begin{example}
% \begin{itemize}[<+->]
% \item If $q=p^{2n}$ and $\#E(\F_p)=q+1\pm2\sqrt{q}=(p^n\pm1)^2$, then
% 
% \alert{\centerline{$E(\F_p)\cong C_{p^n\pm1}\oplus C_{p^n\pm1}.$}}
...
% Let  $E/K: y^2=x^3+Ax+B$, $p\ge5$ and ${D}_E:=4A^3+27B^2$. \pause
% 
...
%   %\ \hfill if $K=\F_p$ can take $u\in\F_{q^{12}}$
...
%  \item $E$ and $E_\mu$ are $\F_p[\sqrt{\mu}]$--affinely equivalent
...
%  \end{columns}\pause
% 
% \begin{definition}[twisted curve] Let $E/\F_p: y^2=x^3+Ax+B, \mu\in\F_p^*\setminus(\F_p^*)^2$.
