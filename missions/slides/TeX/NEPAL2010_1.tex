...
\begin{large}\begin{bf}Algorithmic Complexity \& more.
\end{bf}\end{large}\medskip

July $19^{\textrm{th}}$ 2010\medskip
\vfil
...
If $x\in\Z$, the $\text{size}_b(x)$ is the number of digits of $x$ in base $b$.
That is\pause
$\displaystyle{\text{size}_b(x):= 1+\lceil\log_b(1+|x|)\rceil}$\pause
where $\log_b$ denotes the logarithm in base $b$ and $\lceil u\rceil$ is
the ceiling of $u$ (i.e. the smallest integer greater than or equal to $u$.\pause
...
Example: $2^{1000}\bmod1001 = 561$ implies that $1001$ is not prime and we haven't even tried to factor it
...
and $a^{(n-1)/q}\not\equiv1\bmod n$ for all prime divisors of $n-1$, then $n$ is prime.}
\pause
...
and $\gcd(a^{(n-1)/q}-1,n)=1$ for all prime divisors of $m$, then $n$ is prime.}
\pause
...
\noindent\textbf{Exercise:} Prove that the Fermat number $F_k:=2^{2^k}+1$ is prime iff 
...
\pause
\noindent\textbf{Theorem.}\textit{ The Solovay--Strassen algorithm returns ``\textcolor{Brown}{Prime}'' if $n$
is prime and returns ``\textcolor{Brown}{Composite}'' with probability $\ge 1-2^{-k}$ if $n$ is 
...
produira donc jamais, au moins ne sarait jamais observ\'e.}\bigskip
...
\item (Bach 1990): ERH implies least quadratic nonresidue is $\le 2\log^2p$. (for more on the Riemann Hypothesis
see the course on the Riemann zeta function next week by S. Kanemitsu)
\end{itemize}
\end{slide}
...
\item for each $k=1,\ldots,\lfloor\log_2n\rfloor$, 
