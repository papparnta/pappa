...
\begin{large}\begin{bf}Discrete Logs, Modular Square Roots \& Euclidean Algorithm.
\end{bf}\end{large}\medskip

July $20^{\textrm{th}}$ 2010\medskip
\vfil
...
\parstepwise{\begin{itemize}
 \item \step{Primordial public key cryptography is based on the difficulty of the }\\ \step{Discrete Log problem\hfill \textit{(Cryptography course from Kalyan Chakraborty)}}
\item \step{Several algorithms to compute discrete logarithms are known.}\\ \step{One for all is the \textbf{Shanks Baby Step Giant Step algorithm}.}\end{itemize}}\vspace*{-3mm}
...
is the base of the security of RSA\hfill\textit{(see K. Chakraborty course)}
...
The latter is crucial in RSA encryption and decryption \emph{See. K. Chakrabory course}
\end{slide}
