...
\documentclass[10pt,handout]{beamer} %,hyperref={pdfpagelabels=false},draft,final,,handout
\input{manila-lecture-style.tex}

%\includeonlyframes{current,current1,current2}

\lecture[2]{Elliptic curves over finite fields}{First Steps}
\date{July 24, 2013}

\def\lecturename{CIMPA-ICTP-UNESCO-MESR-MINECO-PHILIPPINES}

\title[Elliptic curves over $\F_{q}$]{\insertlecture}
\subtitle{The Group structure}
...
%\\ (i.e. $\forall f\in\F_q[x], \partial f>1, \exists\alpha\in\overline{\F}_q, f(\alpha)=0);$
         \end{enumerate}
\end{beamercolorbox}
