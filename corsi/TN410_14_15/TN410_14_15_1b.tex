...
% \voffset=0.5cm
% \parskip=-.1cm
\  \hskip -6mm TN410 AA14/15\ (Elementary Number Theory)\hfill Midterm Exam \hfill Rome, April 1, 2015. \hrule
\bigskip\noindent
...
\item{1.} Compute $\gcd(1380,1110)$ using the Extended Euclidean Algorithm and deduce a Bezout Identity.

\item{2.} Compute the $7$--adic valuation $v_7(100!)$.
...
\qquad\qquad \hfil{\it (suggestion: try first to prove the formula for $k=2, 3, ...$)}

\item{4.} After having stated Gauss Theorem of existence of primitive roots modulo integers, 
compute all primitive roots modulo $686$.

\item{5.} Find all integers $X$ in the interval $[-10,200]$ such that $\cases{X\equiv 3\bmod 4\cr X\equiv 2\bmod 5\cr X\equiv 4\bmod 7.}$
