\magnification=1200
\centerline{CR410 AA11/12\ (Crittografia 1)}\bigskip
...
\itemitem{a.} E' vero che tutte le curve ellittiche sono non singolari?
\itemitem{b.} Fornire un esempio di una curva ellittica su un campo finito con gruppo dei
punti razionali non ciclico.
\itemitem{c.} Determinare le radici primitive (i.e. generatori) in ${\bf F}_2[\alpha]$ dove $\alpha^4=1+\alpha$.
\itemitem{d.} E' vero che in ${\bf F}_q[X]$ esistono polinomi irriducibili di ogni grado?
...
\itemitem{a.} E' vero che l'algoritmo Pohlig--Hellman si applica a qualsiasi gruppo finito ciclico?
\itemitem{b.} Quale \`e la probabilit\`a che dati $(x_1,\ldots,x_{100})\in({\bf Z}/500{\bf Z}^{100})$ 
ci siano $i\neq j$ tali che $x_i= x_j$?
\itemitem{c.} Che differenza c'\`e tra polinomi irriducibili e polinomi primitivi?
\itemitem{d.} E' vero che in ${\bf F}_p[X]$ due polinomi di grado $30$ si moltiplicano in $O(\log^2p)$ operazioni bit? \footnote{*}{ESERCIZIO RELATIVO AL PROGRAMMA NON SVOLTO NELL'AA 2013/2014} 
...
\itemitem{c.} Quanti sono i polinomi primitivi di grado minore di $5$ in ${\bf F}_2[X]$?
\itemitem{d.} E' vero che in ${\bf F}_7[X]$ due polinomi di grado $n$ si moltiplicano in $O(n^3)$ operazioni bit?
...
\item{1.} Rispondere alle seguenti domande che forniscono una giustificazione di 1 riga:
\itemitem{a.} Determinare due fattori propri di $$10000000000000000000400000000000000000003$$
\itemitem{b.} \`E vero che $X^{97}-X+3$ non ha radici monulo $97$?
\itemitem{c.} Quanti sono i polinomi primitivi di grado $7$ su ${\bf F}_7$?
